...
#define HAL_MCR_RD(_prAdapter, _u4Offset, _pu4Value) \
	...
	if (HAL_TEST_FLAG(_prAdapter, ADAPTER_FLAG_HW_ERR) == FALSE) { \
		...
		DBGLOG(HAL, ERROR, ("HAL_MCR_RD access fail! 0x(_u4Offset): 0x(_pu4Value)\n", _u4Offset, *_pu4Value)); \
	    } \
	} else { \
	    DBGLOG(HAL, WARN, ("ignore HAL_MCR_RD access! 0x(_u4Offset)\n", _u4Offset)); \
	} \
    }
...
#define HAL_MCR_WR(_prAdapter, _u4Offset, _u4Value) \
	...
	if (HAL_TEST_FLAG(_prAdapter, ADAPTER_FLAG_HW_ERR) == FALSE) { \
		...
		HAL_SET_FLAG(_prAdapter, ADAPTER_FLAG_HW_ERR); \
		fgIsBusAccessFailed = TRUE; \
		DBGLOG(HAL, ERROR, ("HAL_MCR_WR access fail! 0x(_u4Offset): 0x(_pu4Value)\n", _u4Offset, _u4Value)); \
	    } \
	} else { \
	    DBGLOG(HAL, WARN, ("ignore HAL_MCR_WR access! 0x(_u4Offset): 0x(_pu4Value)\n", _u4Offset, _u4Value)); \
	} \
     }
